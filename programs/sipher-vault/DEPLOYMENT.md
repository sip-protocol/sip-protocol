...
- Config: 10 bps fee (stored as fee_bps = 10 under the whole-bps unit in effect at deploy), 86400s refund timeout
...
- **`migrate_config` TX:** `2eUimGuTYRU4Biy6A8jnTjLLpL4p89ccJKddCy2KdSyYzS583onav3HiTfQRwP84iA2n4j9PHK5uZGQsopjmr1xd` — `VaultConfig` `CpL4qy…` 68 → 101; verified fields preserved (authority `FGSkt8…`, fee 10 bps (fee_bps = 10 under the then-current whole-bps unit, preserved byte value), refund_timeout 86400, total_deposits 2, total_depositors 1, bump 254) with `pending_authority = None` (byte 68 = 0x00, trailing zero-filled).
...
Step 2 — reset the fee to 75 (7.5 bps) immediately after the deploy:

```bash
ANCHOR_WALLET=~/Documents/secret/solana-devnet.json \
ANCHOR_PROVIDER_URL=https://api.devnet.solana.com \
pnpm exec tsx scripts/update-fee.ts 75
```

`update-fee.ts` is authority-only, validates `0 ≤ fee ≤ MAX_FEE_TENTHS_BPS` (1000), and
re-reads the config to confirm the stored value — closing the semantic-reset window in a
single command (no hand-assembled Borsh).
